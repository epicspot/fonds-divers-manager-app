...
import { ReferenceModal } from "@/components/dashboard/modals/ReferenceModal";
...
export function ReferencesSection() {
  const { references, loading, createReference, updateReference, deleteReference } = useReferences();
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer cette liste de référence ?")) {
  ...
  if (loading) {
    return (
      <div className="p-6 bg-gradient-to-br from-indigo-50 to-purple-50 h-full flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600 mx-auto mb-4"></div>
          <p className="text-gray-600">Chargement des listes de référence...</p>
        </div>
      </div>
    );
  }
            ...
            <Card key={reference.id} className="h-fit shadow-lg border-0 bg-white/80 backdrop-blur hover:shadow-xl transition-all duration-300 hover:scale-105">
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-gray-600">
                  <span className="font-medium text-indigo-600">{reference.nombre_elements} éléments</span>
                </CardDescription>
              </CardHeader>
              <CardContent className="pt-0">
                <div className="space-y-3">
                  <p className="text-sm text-gray-600 bg-gray-50 p-2 rounded">
                    Modifié le: {new Date(reference.date_modification).toLocaleDateString('fr-FR')}
                  </p>
                  ...
                  <div className="flex gap-2">
                    ...
                    <ReferenceModal
                      ...
                      reference={reference}
                      onSubmit={(data) => updateReference(reference.id, data)}
                      isEdit={true}
                    />
                    <Button 
                      variant="outline" 
